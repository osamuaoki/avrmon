...
BAUD = 19200
#BAUD = 9600
TERM_PORT = /dev/ttyACM0    # test device as serial device
TERM_COMM = picocom -b $(BAUD) $(TERM_PORT)
...
HID_PROGRAMMER = sudo hid_bootloader_cli
REMOVE = rm -f
REMOVEDIR = rm -rf
COPY = cp
WINSHELL = cmd
...
SED = sed
REFORMAT = clang-format -style=Google -i
...
# Reformat C source
reformat:
	$(REFORMAT) $(wildcard *.c) $(wildcard *.h)

# Open terminal session
term:
	$(TERM_COMM)

# program and run it in terminal
run: program sizeafter term

# Create the header file from the template file.
...
clean: begin reformat clean_list end
